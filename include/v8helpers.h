 ...
 */
template <typename G, typename S> struct ParamContainer
{
    std::string name_;
    G getter_;
    S setter_;
...
v8::Local<v8::Object> exposeToV8(std::string object_name, R *object_ref,
                                 std::vector<ParamContainer<G, S>> list_of_params, v8::Isolate *isolate,
                                 v8::Local<v8::Context> &context, std::string context_name = "default")
{
#if V8H_DEBUG
    ...
    object_template->SetInternalFieldCount(
        1); // Internal fields are those only accessible in C++...in this case, this object (at least when complete)
    for (ParamContainer p : list_of_params)
    {
        object_template->SetAccessor(v8::String::NewFromUtf8(isolate, p.name_.c_str()).ToLocalChecked(), p.getter_,
                                     p.setter_);
    }
